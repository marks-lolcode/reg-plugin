...
(async function triggerIconUpdate() {
  ...
  const note = getRegistrationNote();
  await chrome.storage.local.set({ [STORAGE_KEY.REGISTRATIONS]: { data, note } });
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === ACTION.GET_REGISTRATIONS) {
    sendResponse({ data: getRegistrationsInfo(), note: getRegistrationNote() });
  }
});
...
// ── NOTE HELPER ────────────────────────────────────────────────────────
// Reads the registration note from div#noteContent, if present.
// Returns the trimmed text, or "" if there is no note.
function getRegistrationNote() {
  return document.getElementById("noteContent")?.textContent?.trim() ?? "";
}
...
function getRegistrationsInfo() {
  ...
  if (attendeeRows.length === 0) {
    ...
    return [{
      accountId: "", neonAttendeeId: "", legalName: "Page Error", preferredName: "",
      state: STATE.RED,
  ...
  const results = Array.from(attendeeRows).map((td, i) => {
    ...
    return buildRegistrantState({
      ...
      activeBadges, iceContact, regHold, artHold, opsHold,
    });
  });
...
function buildRegistrantState({
  ...
  // Collect every triggered condition: key ₒ { text, isRed }
